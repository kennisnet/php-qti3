...
**Assessment tests** (`src/AssessmentTest/`): `TestBuilder` assembles an `AssessmentTest` from a parsed package. `AssessmentTestParser` delegates to `OutcomeDeclarationParser`, `TestPartParser`, `RubricBlockParser` (test-level `qti-rubric-block`), `OutcomeProcessingParser` (`qti-outcome-processing`, expressions via `QtiExpressionParser`) and `TestFeedbackParser` (`qti-test-feedback`).
